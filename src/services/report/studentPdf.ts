...
export interface StudentPdfStats {
...
/**
 * Opens a print-ready, Asiacell-branded report for one student in a new tab
 * and triggers the browser's print dialog so the lecturer can save it as PDF.
 * The logo is loaded from /asiacell-logo.svg (replace that file in `public/`
 * with the official artwork to rebrand the export).
 */
export function exportStudentPdf(
  student: Student,
  stats: StudentPdfStats,
  rows: StudentPdfRow[],
): void {
  const win = window.open('', '_blank');
  if (!win) return;
  win.document.write(buildHtml(student, stats, rows));
  win.document.close();
}
...
function buildHtml(
  ...
  const meta = [student.college, student.department]
    .filter(Boolean)
    .join(' · ');
...
<head>
<meta charset="utf-8" />
<title>Attendance — ${esc(student.fullName)}</title>
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
  ...
  .student { display: flex; align-items: baseline; gap: 10px; margin: 18px 2px 4px; }
...
<body>
  ...
  <div class="student">
    ...
    <span class="code">${esc(student.code)}</span>
  </div>
  <p class="meta">${esc([meta, student.phone].filter(Boolean).join(' · '))}</p>
  ...
  <table>
    <thead>
      <tr><th>Lecture</th><th>Date</th><th>Check-In</th><th>Check-Out</th><th>Status</th><th>Hours</th></tr>
    </thead>
  ...
  <div class="foot">
    ...
    <span>${esc(student.fullName)} (${esc(student.code)})</span>
  </div>
