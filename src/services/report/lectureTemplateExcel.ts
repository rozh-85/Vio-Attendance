...
export function buildLectureAttendanceXlsxBlob(
  ...
  return createXlsx([
    ...
    { path: 'docProps/core.xml', content: coreXml(session) },
    { path: 'xl/workbook.xml', content: workbookXml(lastRow) },
    { path: 'xl/_rels/workbook.xml.rels', content: workbookRelsXml() },
    { path: 'xl/styles.xml', content: stylesXml() },
    { path: 'xl/theme/theme1.xml', content: themeXml() },
    {
      path: 'xl/worksheets/sheet1.xml',
      content: worksheetXml(session, rows, lastRow),
    },
  ]);
}
...
function worksheetXml(
  ...
  const sheetRows = [
    ...
    rowXml(
      ...
      [
        textCell('A', 4, 'Date', 2),
        textCell('B', 4, formatDate(session.startedAt), 3),
      ],
      24,
    ),
    rowXml(
      5,
      [
        textCell('A', 5, 'Location', 2),
        textCell('B', 5, session.location, 3),
      ],
      24,
    ),
    rowXml(
      6,
      [
        textCell('A', 6, 'Start Time', 2),
        textCell('B', 6, formatClock(session.startedAt), 3),
      ],
      24,
    ),
    rowXml(
      7,
      [
        textCell('A', 7, 'End Time', 2),
        textCell('B', 7, session.closedAt ? formatClock(session.closedAt) : 'In progress', 3),
      ],
      24,
    ),
    rowXml(
      ...
      [
        ...
        textCell(
          'B',
          8,
          formatDuration(session.startedAt, session.closedAt ?? new Date().toISOString()),
          3,
        ),
      ],
      24,
...
function coreXml(session: Session): string {
